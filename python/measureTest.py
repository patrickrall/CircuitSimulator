...
from circuit.gadgetize import expandGenerators
import numpy as np
import json
...
zs = [np.array([1.0, 1.0]), np.array([1.0, 1.0])]

(phases, xs, zs) = expandGenerators((phases, xs, zs))
print(phases)
print(xs)
print(zs)
...
def printFull(state):
    print("n", state.n)
    print("k", state.k)
    print("G", json.dumps(state.G.tolist()))
    print("Gbar", json.dumps(state.Gbar.tolist()))
    print("Q", state.Q)
    print("D", json.dumps(state.D.tolist()))
    print("J", json.dumps(state.J.tolist()))
